...
class WCGANTrainer:    
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (real_images, right_emb, wrong_emb)  in enumerate(self.dataloader):
            ...
            real_valid = self.dis(real_images, right_emb)
            fake_valid = self.dis(fake_images, right_emb)
            wrong_valid = self.dis(real_images, wrong_emb)

            real_loss = -torch.mean(real_valid)
            fake_loss = torch.mean(fake_valid)
            wrong_loss = torch.mean(wrong_valid)

            D_x = real_valid.mean().item()
            D_G_z1 = fake_valid.mean().item()
